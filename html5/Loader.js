...
function postInstantiate(baseModule, instance) {
	...
	function newFunction(fn) {
		if (typeof fn.original === "function") {
			fn = fn.original;
		}
...
function instantiate(module, imports) {
	var baseModule = preInstantiate(imports || (imports = {}));
	var instance = new WebAssembly.Instance(module, imports);
	return postInstantiate(baseModule, instance);
}
...
async function instantiateStreaming(response, imports) {
	var baseModule = preInstantiate(imports || (imports = {}));
	var instance = (await WebAssembly.instantiateStreaming(response, imports)).instance;
	return postInstantiate(baseModule, instance);
}
...
function demangle(exports, baseModule) {
	...
	for (let internalName in exports) {
		if (!hasOwnProperty(exports, internalName)) {
			continue;
		}
			...
			if (typeof classElem === "undefined" || !classElem.prototype) {
				...
				if (classElem) {
					Object.getOwnPropertyNames(classElem).forEach(
						name => Object.defineProperty(ctor, name, Object.getOwnPropertyDescriptor(classElem, name))
					);
				}
				curr[className] = ctor;
			}
